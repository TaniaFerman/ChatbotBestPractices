...
Develop a set of best practices that allow designers to create and design a chatbot.
...
% WIRTE A PARAGRAPH TO DESCRIBE THE TWO DIFFERENT PERSONAS. 
...
Conversational Knowledge is when the user is able to follow the meaning of the chatbot's remarks, while the chatbot is able to select a response that fits coherently into what the user is saying. Therefore, it is vital to understand how chatbots and users follow each others' thoughts. In other words, Conversational Knowledge are the common definitions* in the conversation that do not change overtime. Figure ~\ref{FigurePoncho} shows the user asking to Poncho (the weather forecast chatbot) to have the temperature in Celsius rather than Fahrenheit degrees. In this example temperature units are the shared conversational knowledge. For this reason the user request make sense in the conversational context, and the chatbot is able to make the conversion of the temperature.     
...
Situational Knowledge is another important part for having a mutual understanding in a conversation 
Situational Context involves previous knowledge about the topic that is being discussed.  
...
shows a users asking "What is the temperature in London right now? Due to Poncho (the weather forecast chatbot) and the user share the same situational knowledge about the current time, the provided forecast will be according to when the user ask the information. 
