...
\textbf{The second phase} is the creation of a customer support chatbot, designed according to the best practices of the first phase. The chatbot will be designed to support the customer service of Shift. 
Shift is a consumer application that enables users to improve their email productivity. Users can have their Gmail, Outlook and Microsoft 365 account in one application. In addition, Shift also allows extensions and apps that can be used  in the work environment. Therefore, users can switch between emails, apps, and extensions, creating  an organized work environment, thus allowing them to be more efficient.  

Shift is an application created by Redbrick. Redbrick is a software company founded in 2011 that develops software for customers and businesses. 

Finally, \textbf{the third phase} is the case study. This study will be carried out to evaluate the best practices and the chatbot. Developers from Redbrick will evaluate them and give feedback. After this, the best practices and the chatbot will be updated in order to address the possible flaws that the developers found. At the end, the developers will have a final evaluation. 
...
\textbf{Chatbot:} The goal of the chatbot is to help in the customer support process of a software company. Customers can use natural language statements in order to communicate with the chatbot. In response, the chatbot will provide them the most relevant answer for their request.

\textbf{Chatbot's Tasks on Customer support:} According to Cui et al.~\cite{cui2017superagent} ``Customer service plays an important role in an organization's ability to generate income and revenue. It is often the most resource-intensive department within a company, consuming billions of dollars a year to change the entire perception customers hold." 

Customer support is not a simple task, staff need to have an accurate knowledge of the information given to the users, and at the same time be professional and friendly. Even when customers are giving a negative feedback the staff needs to behave appropriately and try to deal with the complaint in the best manner they can. Another important aspect of customer services that is very hard to accomplish is to give users 24/7 service. This last aspect is difficult to have because usually employeesare hired for an 8 hours schedule. International companies have to deal with giving feedback to customers within different time zones, making the customer service process expensive. 

A great solution for all these problems is chatbots. Chatbots can deal with repetitive tasks and give customers a 24/7 service. Companies can expense fewer resources in staff by adding a chatbot to the customer support. Therefore, the staff will manage only the tasks that the chatbot is not capable to deal. Making their job more efficient and less time-consuming.

The customer support goal of the chatbot is to help users get the assistant that they need. The chatbot needs to answer the customer questions in the most accurate way. At the beginning, the chatbot will offer different options of the questions that can be answered. This will function as a menu of the information that the chatbot can convey. 

The creation of a customer support chatbot allows us to test the best practices of this project's first phase. A customer support chatbot was selected because providing online customer service is an emerging area that allows companies to improve their communication with customers. One example of a chatbot used for customer support is Super Agent\cite{cui2017superagent}. Super Agent is a customer service chatbot that replies customer questions base on a Questions and Answers engine that contains the information regarding the company's product.     
...
%When chatbots need to provide different options, from which the user need to select one, the use of a carousel is the most efficient way. 
...
\section*{\centering{Customer Support Chatbot}}

An early meeting with Nadia Tatlow, the Manager of Marketing at Redbrick, was carried out to gather information about the Shift users' demographics.  According to Tatlow, the shift users are mostly men (90\%) between the age of 18 to 44 years old. The users are mostly from the US and they are early adopters that have a good understanding of design and different software technologies (Tech-savvy). They use Shift in order to become more efficient in their jobs and have a better use of their time.  Figure \ref{ShiftDemographics} shows two bar charts, the first chart shows the gender and age demographics, and the second bar chart exhibit the time when shift customer uses Shift. Besides a choropleth map is being displayed, this graph conveys the different regions where Shift users live. 

The chatbot will be designed to aim that target market and demographics. The chatbot should use words, expressions, and jokes relevant to the target market. Besides, the chatbot should address the most frequent questions asked by them. 

Customer Support chatbot: 

\begin{figure}
\centering
\includegraphics[scale=0.5]{/Users/MariaFerman/Desktop/ToRead(Project:Bots)/imgDesignfesst/ShiftDemographics.png}
\caption{Shift Demographics}
\label{ShiftDemographics}
\end{figure}
